...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($username && $role) {
    ...
    if ($stmt->execute()) {
      ...
      $message = "Gagal menambahkan operator: " . $conn->error;
    }
  } else {
...
<body class="bg-gray-100 font-sans text-gray-800">
          ...
          <tr class="border-b hover:bg-gray-50">
            ...
            <td class="py-3 px-4">
              <?php if (!empty($row['img'])): ?>
                <img src="../app/uploads/<?= htmlspecialchars($row['img']) ?>" class="w-10 h-10 rounded-full object-cover">
              <?php else: ?>
                <div class="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center text-xs text-gray-600">N/A</div>
              <?php endif; ?>
            </td>
  ...
  <div id="modal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative">
      <h2 class="text-xl font-semibold mb-4 text-center">Tambah Operator</h2>
      ...
      <form method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
          ...
          <input type="file" name="img" accept="image/*" class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none">
        </div>
